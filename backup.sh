...
if [ -s $status_file ]; then
	log_entry="Status file not empyu, remaining dumps: $(cat $status_file)" send_log
fi
...
#Creating remote folders if needed
if ($ssh '[ ! -d "$dbfilepath" ]'); then
...
if [ $? != 0 ]; then
	...
	if [ $is_ftp -eq 1 ]; then
	#Sets ftp settings on remote file before next step
	set_ftp_settings
	fi			
...
function handle_error {
	log_entry="Something went wrong, aborting script and removing $db." send_log
	remove_local_fail_obj
	check_status_file
...
trap handle_error SIGINT SIGHUP SIGQUIT SIGILL SIGTRAP SIGABRT
